...
def get_best_trip_per_date(conn):
    ...
    return conn.execute("""
        ...
        FROM trip_prices tp
        WHERE timestamp = (
            SELECT MAX(timestamp) FROM trip_prices WHERE departure_date = tp.departure_date
        )
...
def scrape_google_flights_roundtrip(origin, destination, dep_date, ret_date):
...
def browse_flexible_trip(conn):
    ...
    for day_offset in search_points:
        ...
        for dep_str, ret_str, td in flex_windows:
            result = scrape_google_flights_roundtrip(ORIGIN, DESTINATION, dep_str, ret_str)
...
def run():
    ...
    best_trip, all_results = browse_flexible_trip(conn)

    if not best_trip:
        print(json.dumps({"error": "No trip data found", "timestamp": timestamp}))
        conn.close()
        return
    ...
    save_trip_price(
        ...
        best_trip["trip_days"],
        best_trip["price"],
        notes=f"Best of {len(all_results)} flexible windows searched"
    )
    ...
    # 4. Build report
    report = build_report(conn, timestamp, best_trip, all_results, prev_price)
...
def build_report(conn, timestamp, best_trip, all_results, prev_price):
    ...
    report = {
        ...
        "total_windows_searched": len(all_results),
